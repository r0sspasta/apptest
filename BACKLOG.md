...
## Weight increments — built

Per-exercise increments, defaulted from the smallest weight change in that
exercise's history, plus tap-to-type for exact values. Remaining:

- The increment is stored in kg, so a value chosen in one unit shows as an
  odd number in the other (2.5 kg reads as 5.51 lb). It is preserved rather
  than reset, but the presets won't match.
- No quick "bigger jump" button; a large change means several taps or typing
  the value.
